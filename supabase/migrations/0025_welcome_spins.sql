-- ============================================================================
--  HOUSE LOOT — 0025  THE FIRST FOUR SPINS ALWAYS GIVE SOMETHING
-- ============================================================================
--  The failure mode that actually loses the party is not bad odds, it is a
--  player putting in $5, losing the first two spins to scrap coins, and never
--  opening the app again. A guaranteed opening run costs the house a handful of
--  the cheap items it was going to bin anyway and buys every player a reason to
--  keep going.
--
--  The rolls are still PAID FOR. What is guaranteed is that they return a real
--  object rather than coins, a re-roll or a shard.
--
--  Scoped deliberately:
--    * Only tier_0 and tier_1. Without that, the first thing anyone sensible
--      does is spend the guarantee on four $50 boxes.
--    * Never purple/pink/gold. A floor, not a jackpot -- the headline prizes
--      still have to be won.
--    * Falls back to an ordinary roll if nothing eligible is in stock, rather
--      than erroring or promising an item that does not exist.
--
--  NOTE this makes a beginner's real odds better than the published ones, which
--  is the reverse of the usual danger but still a gap between what the table
--  says and what happens. The UI says so explicitly on the box screen.
-- ============================================================================

UPDATE public.config
   SET value = value || jsonb_build_object('welcome_spins', 4)
 WHERE key = 'settings' AND NOT (value ? 'welcome_spins');
