-- ---------------------------------------------------------------------------
--  0037 — the shard curve was published but never played
--
--  0017 introduced progressive shard difficulty and gave box_odds a second
--  parameter so the rate could depend on how many shards the caller already
--  holds. Its own comment states the intent plainly: "the shard EV charged to
--  a roll must be the one THAT player actually faces."
--
--  open_box never passed it.
--
--      odds := public.box_odds(p_box_tier);      -- p_user_id defaults to NULL
--
--  With p_user_id NULL the curve block is skipped entirely and the BASE rate
--  is used for every shard. So the odds page showed one number and the server
--  rolled another:
--
--                       published to a 3-of-4 holder   actually rolled
--      tier_3  $30              1.001%                     35%
--      tier_2  $10              0.200%                      7%
--      tier_1  $3               0.100%                    3.5%
--
--  The last shard was 35x more likely than the number next to it on screen,
--  and the set was a $343 grind instead of the $3,547 the curve describes.
--  Every held shard made the gap worse, because the curve only ever bends the
--  number that was being ignored.
--
--  This is the third instance of the same failure in this codebase: a value
--  computed correctly in one place and read from another that never receives
--  it. The fix is one argument. The reason it deserves a migration of its own
--  is that it changes what the machine costs to win by a factor of ten.
--
--  Nothing else in box_odds reads p_user_id -- only the curve -- so passing it
--  changes the shard rate and the shard EV charged against the budget, and
--  nothing more.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
--  Both pot gates were live at 0, so neither was doing anything.
--
--    pot_revenue_threshold  0 -> 200   shards do not drop until the party has
--                                      actually taken $200 in approved
--                                      deposits. box_odds already enforced
--                                      this and publishes pot_gate_met so a
--                                      card can say WHY the odds read 0%.
--
--    pc_claim_threshold     0 -> 100   four shards can be assembled at any
--                                      time, but the machine does not leave
--                                      the house until the pot reaches $100.
--                                      Refusing a claim does not burn shards.
--
--  0032 designed both gates and reads them on every claim; nothing ever wrote
--  them, so `value->>'pc_claim_threshold'` fell through COALESCE to 0 and the
--  PC was claimable from the first roll of the night.
-- ---------------------------------------------------------------------------

UPDATE public.config SET value = value || jsonb_build_object(
  'pot_revenue_threshold', 200,
  'pc_claim_threshold',    100
) WHERE key = 'settings';
