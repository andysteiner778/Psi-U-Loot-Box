-- ---------------------------------------------------------------------------
--  0050 — PER-TIER SHARD LADDER, AN HONEST SHARD PRICE, AND AN ENDGAME TAPER
--
--  1. SHARD LADDER. One shared `shard_progress_curve` scaled by a per-tier base
--     cannot express "20% and 10% for the first two shards on EVERY box, then
--     diverge for the last two". `shard_ladder` is a per-tier array indexed by
--     shards held, so the Nth entry is the chance of the (N+1)th shard:
--
--         tier_3 High Roller   30%  20%  10%  0.5%
--         tier_2 Golden Chest  20%  10%   2%  0.1%
--         tier_1 Good Stuff    20%  10%   1%  0.05%
--         tier_0 OG Junk Box   20%  10%  0.5%  0.01%
--
--     A full tier_3 set is 1/.30 + 1/.20 + 1/.10 + 1/.005 = ~218 rolls, roughly
--     $6,500. The first three feel reachable; the machine is safe.
--
--     The old base x curve stays as a fallback so a config lacking the ladder
--     keeps working instead of silently dropping shards to zero.
--
--  2. A COMPLETE SET PUBLISHES 0%. The curve held its last multiplier past the
--     end of the array, so a player at 4/4 was still shown a live shard chance.
--     open_box refuses to mint past shards_required and falls through to a
--     refund, so that number was advertising something the server could never
--     award -- and it kept showing after they had collected them all.
--
--  3. shard_ev_value. box_odds charged the budget pc_value/shards_required =
--     $100 per shard. That is more than a $0.50 box costs, so the EV solve gave
--     up and dumped everything into the floor anchor -- measured at 96% junk on
--     tier_1. A shard's honest expected cash value is near zero because a set
--     is almost never completed, so the budget is charged a flat
--     `shard_ev_value` instead. `pc_value` stays the machine's real worth for
--     claim_pc and the HUD.
--
--  4. ENDGAME TAPER. p_shard is a fixed per-tier number, so as the real prizes
--     are won it becomes a larger share of what is left -- the PC gets EASIER
--     exactly as the shelf empties. p_shard is now multiplied by
--     min(1, real_units_left / shard_full_stock_threshold), so a tier down to
--     its last item is a fifth as likely to pay a shard. Combined with the
--     tier lock from 0039/0040 (a tier with no real objects refuses rolls
--     outright, and therefore drops no shards either), a cleaned-out box
--     cannot become a PC lottery.
--
--  5. The floor anchor's award query in open_box omitted
--     `AND NOT bundle_only`, which both pools in box_odds enforce -- so a
--     bundled favor could be handed over on its own as a floor prize, unpriced.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT array_agg(t.id      ORDER BY t.id), array_agg(t.name      ORDER BY t.id),
         ...
         array_agg((COALESCE(
           ...
           ORDER BY t.id)
    INTO ids, nms, vals, rars, scrs, imgs, stks, msrps, tiers, bvals
    FROM public.items t
  ...
  SELECT COALESCE(SUM(t.stock_qty), 0),
         ...
         array_agg((COALESCE(
           ...
           + COALESCE((SELECT b.est_value FROM public.items b WHERE b.id = t.bonus_item_id), 0))
           ORDER BY t.id)
    INTO v_fill_stock, v_fill_value, f_ids, f_nms, f_vals, f_rars, f_imgs, f_stks, f_msrps, f_bvals
    FROM public.items t
...
CREATE OR REPLACE FUNCTION public.open_box(p_user_id uuid, p_box_tier text, p_client_roll_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
DECLARE
  -- A BUNDLED VOUCHER: an item that hands you a free spin or a discount along
  -- with the object itself, so a cheap win still leaves you able to play. Its
  -- cost is charged to the EV budget in box_odds, so this is paid for, not
  -- generosity bolted on afterwards.
  v_freward   NUMERIC;
  v_fvch_tier TEXT;
  v_fvch_pct  NUMERIC;
  v_bon_tier  TEXT;
...
BEGIN
  ...
  -- A box with nothing real left in it must not take the money. box_odds
  -- would still price it and open_box would still refund, so the player pays,
  -- waits for the reel and gets their money back -- indefinitely. Checked
  -- AFTER the profile lock and BEFORE the charge.
  ...
  odds    := public.box_odds(p_box_tier, p_user_id);
  ...
  IF v_pick > 0 THEN
    ...
    IF v_affected = 1 THEN
      ...
      SELECT bonus_voucher_tier, bonus_voucher_pct, bonus_item_id
      ...
      v_result := jsonb_build_object(
        ...
        'msrp', v_it->'msrp',
        'bonus_tier', v_bon_tier, 'bonus_pct', v_bon_pct,
        'bonus_item', CASE WHEN v_bon_item IS NULL THEN NULL ELSE jsonb_build_object(
          'item_id', v_bon_item, 'item_name', v_bi_name, 'rarity', v_bi_rar,
          'est_value', v_bi_val, 'scrap_value', v_bi_scrap,
          'image_url', v_bi_img, 'msrp', v_bi_msrp, 'roll_id', v_bi_roll) END,
  ...
  -- p_box_tier <> 'tier_0', not IN ('tier_2','tier_3'). box_odds publishes a
  -- filler list and a floor_kind for tier_1 as well -- four items worth 4.4% of
  -- that box -- but this branch skipped tier_1, so those rolls silently paid
  -- credit instead. The odds table advertised objects the server would never
  -- hand over, and the EV budget had already been priced as if it did.
  -- tier_0 has no cheaper tier to borrow from, so its floor_kind is 'coins'
  -- and it never enters here anyway.
  IF (odds->>'floor_kind') = 'item' AND p_box_tier <> 'tier_0' THEN
    ...
    SELECT id, name, image_url, rarity, est_value, scrap_value, msrp
     ...
     WHERE array_position(ARRAY['tier_0','tier_1','tier_2','tier_3'], box_tier)
       ...
       -- Mirrors both pools in box_odds. Without it a bundle_only rider -- a
       -- favor that is meant to arrive attached to something else -- could be
       -- handed over as a floor prize on its own, unpriced by the budget.
       AND NOT COALESCE(bundle_only, FALSE)
    ...
    IF FOUND THEN
      ...
      -- REWARD ITEMS ARE NOT OBJECTS. The filler pool is drawn from cheaper
      -- tiers, and those tiers contain house credit, free spins and discount
      -- vouchers -- rows in `items` that are promises, not things. This branch
      -- used to insert whatever it drew as kind='physical', so a player who
      -- lost a $10 roll received a row in their inventory literally named
      -- "$1 House Credit" and never got the dollar. Measured before this fix:
      -- 43 of 400 tier_2 rolls, about 11%, handed over a reward item as an
      -- object -- credit, free spins and 50%-off vouchers all silently eaten.
      --
      -- They stay IN the pool, because a losing roll that pays credit or a free
      -- spin is the retention the cheap tiers are built around, and box_odds
      -- prices them into the floor anchor either way. They are simply PAID
      -- correctly now.
      IF v_affected = 1 THEN
        SELECT reward_credit, reward_voucher_tier, reward_voucher_pct
          INTO v_freward, v_fvch_tier, v_fvch_pct
          FROM public.items WHERE id = v_fid;

        IF v_freward IS NOT NULL AND v_freward > 0 THEN
          UPDATE public.profiles SET balance = balance + v_freward WHERE id = p_user_id;
          INSERT INTO public.rolls (user_id, box_tier, kind, item_id, item_name,
                                    item_rarity, status, box_price, client_roll_id)
          VALUES (p_user_id, p_box_tier, 'respin', v_fid, v_fname, v_frar,
                  'consumed', v_price, p_client_roll_id)
          RETURNING id INTO v_roll_id;
          v_result := jsonb_build_object(
            'type','respin', 'item_id', v_fid, 'item_name', v_fname, 'rarity', v_frar,
            'image_url', v_fimg, 'refund_amount', v_freward, 'roll_id', v_roll_id);
          UPDATE public.rolls SET payload = v_result WHERE id = v_roll_id;
          RETURN v_result;
        END IF;

        IF v_fvch_tier IS NOT NULL THEN
          INSERT INTO public.rolls (user_id, box_tier, kind, item_id, item_name,
                                    item_rarity, status, box_price, client_roll_id)
          VALUES (p_user_id, p_box_tier, 'respin', v_fid, v_fname, v_frar,
                  'consumed', v_price, p_client_roll_id)
          RETURNING id INTO v_roll_id;
          INSERT INTO public.vouchers (user_id, box_tier, discount_pct, source_roll_id)
          VALUES (p_user_id, v_fvch_tier, v_fvch_pct, v_roll_id);
          v_result := jsonb_build_object(
            'type','respin', 'item_id', v_fid, 'item_name', v_fname, 'rarity', v_frar,
            'image_url', v_fimg, 'refund_amount', 0,
            'voucher_tier', v_fvch_tier, 'voucher_pct', v_fvch_pct, 'roll_id', v_roll_id);
          UPDATE public.rolls SET payload = v_result WHERE id = v_roll_id;
          RETURN v_result;
        END IF;
        ...
        SELECT bonus_voucher_tier, bonus_voucher_pct INTO v_bon_tier, v_bon_pct
          FROM public.items WHERE id = v_fid;
        IF v_bon_tier IS NOT NULL THEN
          INSERT INTO public.vouchers (user_id, box_tier, discount_pct, source_roll_id)
          VALUES (p_user_id, v_bon_tier, v_bon_pct, v_roll_id);
...
-- ---------------------------------------------------------------------------
--  Config: the ladder, the honest shard price, the endgame taper threshold,
--  and the free re-roll cap.
--
--  max_respin_share 0.25 -> 0.05 is the single lever behind "the free roll
--  token shows up far too often". It works by raising the item scale factor
--  until 1 - p_shard - p_physical <= the cap, so the freed probability lands on
--  REAL ITEMS, not on more junk. Measured before: tier_3 52.5% re-roll, and 32%
--  of 47 test rolls came back as Free Re-Roll Token.
--
--  The margins are in est_value terms, which is close to meaningless when the
--  goods are priced at $0.01 to be given away. Their only job here is to stop
--  the solver inventing payouts the catalogue cannot fund. What a player
--  actually receives is unchanged: an item.
-- ---------------------------------------------------------------------------

UPDATE public.config SET value = value || jsonb_build_object(
  'shard_ladder', jsonb_build_object(
    'tier_0', jsonb_build_array(0.20, 0.10, 0.005, 0.0001),
    'tier_1', jsonb_build_array(0.20, 0.10, 0.01,  0.0005),
    'tier_2', jsonb_build_array(0.20, 0.10, 0.02,  0.001),
    'tier_3', jsonb_build_array(0.30, 0.20, 0.10,  0.005)
  ),
  'shard_ev_value',              0.25,
  'shard_full_stock_threshold',  5,
  'max_respin_share',            0.05,
  'house_margin',                0.70,
  'tier_margins', jsonb_build_object(
    'tier_0', 0.40, 'tier_1', 0.60, 'tier_2', 0.70, 'tier_3', 0.80
  )
) WHERE key = 'settings';
