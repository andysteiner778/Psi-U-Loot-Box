-- ============================================================================
--  HOUSE LOOT — 0012  SHARD-LOCKED ITEMS + ADMIN GIFTS
-- ============================================================================
--
--  SHARD-LOCKED ITEMS
--  Shards were a single hard-coded track ending in one PC. The owner wants
--  several prizes behind shards at different costs -- a monitor for 2, the PC
--  for 4 or 5 -- so `shard_cost` moves onto the item itself.
--
--  An item with shard_cost > 0 is REMOVED from the box drop pool. That is the
--  point: it is not something you can luck into on a $50 roll, it is something
--  you assemble toward. Leaving it in both places would let a player win the PC
--  outright while someone else was three shards deep, which is precisely the
--  feels-bad the shard track exists to avoid.
--
--  It also fixes the live catalog: the $400 Gaming PC was sitting in the tier-3
--  pool at a 2.5% direct drop chance.
--
--  ADMIN GIFTS
--  A way to hand someone free spins -- for a good sport, a bad beat, or helping
--  carry the sofa. Credited as balance so it flows through the same accounting
--  as a deposit, but recorded separately so gifts are never mistaken for money
--  that was actually paid in. That distinction matters: the pot gate and the
--  house's books both key off real deposits.
-- ============================================================================

ALTER TABLE public.items ADD COLUMN IF NOT EXISTS shard_cost INT NOT NULL DEFAULT 0
  CHECK (shard_cost >= 0);

COMMENT ON COLUMN public.items.shard_cost IS
  'Shards needed to claim. 0 = ordinary box drop. >0 removes it from the drop pool.';

-- Gift ledger. Separate from `deposits` on purpose: a gift is not revenue, must
-- not count toward the pot gate, and must not look like money in the books.
...
-- ---------------------------------------------------------------------------
--  Keep shard-locked items out of the drop pool
-- ---------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.claim_with_shards(p_user_id UUID, p_item_id UUID)
RETURNS JSONB
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $fn$
DECLARE v_item RECORD; v_have INT;
BEGIN
  SELECT id, name, est_value, rarity, image_url, shard_cost, stock_qty, is_active
    INTO v_item FROM public.items WHERE id = p_item_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No such item' USING ERRCODE = 'PT404';
  END IF;
  IF v_item.shard_cost <= 0 THEN
    RAISE EXCEPTION 'That item is not claimable with shards' USING ERRCODE = 'PT409';
  END IF;
  IF NOT v_item.is_active OR v_item.stock_qty < 1 THEN
    RAISE EXCEPTION 'Already claimed by someone else' USING ERRCODE = 'PT409';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No such player' USING ERRCODE = 'PT404';
  END IF;
  IF v_have < v_item.shard_cost THEN
    RAISE EXCEPTION 'Need % shards, you have %', v_item.shard_cost, v_have
      USING ERRCODE = 'PT402';
  END IF;

  -- Conditional decrement: two people spending their last shards on the same
  -- monitor at once must not both get one.
  UPDATE public.items SET stock_qty = stock_qty - 1
  ...
  UPDATE public.profiles SET pc_shards = pc_shards - v_item.shard_cost
   WHERE id = p_user_id;

  INSERT INTO public.rolls (user_id, box_tier, kind, item_id, item_name, item_rarity,
                            status, box_price)
  VALUES (p_user_id, 'tier_3', 'physical', v_item.id, v_item.name, v_item.rarity,
          'claimed', 0);

  RETURN jsonb_build_object('ok', true, 'item_name', v_item.name,
                            'spent', v_item.shard_cost, 'value', v_item.est_value);
END;
$fn$;

-- ---------------------------------------------------------------------------
--  admin_grant_spins — free rolls as a gift
-- ---------------------------------------------------------------------------
...
REVOKE EXECUTE ON FUNCTION public.claim_with_shards(UUID, UUID)                        FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.admin_grant_spins(UUID, UUID, TEXT, INT, TEXT)       FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.claim_with_shards(UUID, UUID)                        TO service_role;
GRANT  EXECUTE ON FUNCTION public.admin_grant_spins(UUID, UUID, TEXT, INT, TEXT)       TO service_role;
