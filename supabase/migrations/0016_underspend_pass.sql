-- ============================================================================
--  HOUSE LOOT — 0016  MIRROR THE UNDERSPEND PASS INTO SQL
-- ============================================================================
--  lib/economy.ts gained a third scale pass that fixes the house quietly
--  keeping MORE than its intended margin when a tier is full of items cheaper
--  than the box. That pass was never mirrored here.
--
--  So the two engines disagreed by $11.74 of expected payout on a $50 box --
--  production charging real players a margin the solvency proof said was 12.5%
--  while it was closer to 33%. The proof runs on the TypeScript engine, so it
--  saw nothing wrong; only a direct output comparison between the two could
--  find this, and that check did not exist until now.
--
--  The lesson is in scripts/verify-sql.ts: a first version of that comparison
--  passed even with a known bug deliberately reintroduced, because the seed
--  catalog did not contain the shape that triggers it. It now inserts fixtures
--  that force the edge cases.
-- ============================================================================
