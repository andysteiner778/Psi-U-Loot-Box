-- ============================================================================
--  HOUSE LOOT — 0018  PUBLISH THE FILLER ODDS
-- ============================================================================
--  box_odds returned the tier's own prizes but never the filler pool -- the
--  cheap items borrowed in as the consolation slot. The UI has a `filler` field
--  and renders it; the SQL simply never populated it, so it was always empty.
--
--  The effect was that the published odds omitted the MOST COMMON outcomes. In
--  a $50 box the filler branch is roughly a quarter of all rolls, and every one
--  of those items was invisible: a player reading the odds table saw
--  percentages that did not add up to 100 and no sign of the grey items they
--  kept actually winning.
--
--  Probabilities are conditional on the floor branch being drawn and weighted
--  by stock, so a pile of eight cables is eight times likelier than one lamp.
-- ============================================================================
