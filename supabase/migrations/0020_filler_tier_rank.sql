-- ============================================================================
--  HOUSE LOOT — 0020  FILLER COMES FROM CHEAPER TIERS ONLY
-- ============================================================================
--  Adding tier_0 exposed a flaw in the filler rule. It took any item from
--  another tier under a flat $15 cap, so the $5 box could borrow $15 tier-2
--  items as its "cheap consolation". The floor anchor came out at $5.41 against
--  a $4.38 payout budget, and tier 1 paid out MORE than it took in -- a
--  realized margin of -18.5% on every roll.
--
--  Filler is junk borrowed UP the ladder. Ranking by tier says that explicitly
--  and survives future re-pricing, where a fixed cap does not.
--
--  Caught by `npm run audit` against the live catalog, not by any offline gate:
--  the fixture catalog happens to contain no $15 item in tier 2.
-- ============================================================================
