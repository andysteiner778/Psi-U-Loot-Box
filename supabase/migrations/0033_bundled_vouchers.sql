-- ---------------------------------------------------------------------------
--  0033 — bundled vouchers: win the thing AND keep playing
--
--  A cheap object on its own is a flat note to end a spin on. Bundling a free
--  spin or a half-price voucher with it means a small win still leaves you able
--  to play, which is the whole point of a junk-heavy catalogue: the objects are
--  being given away anyway, so the value that keeps someone at the machine
--  should ride along with them rather than replace them.
--
--  THE BUNDLE IS PAID FOR, NOT FREE. `bonus_value` = the bonus tier's box price
--  x the discount, and box_odds now folds it into BOTH halves of the solve:
--
--    * the weight formula  min(cap, C*f / (V + bonus))  -> bundled items are
--      correspondingly rarer, exactly as a dearer item would be;
--    * the EV budget       Wv += w * (V + bonus)        -> the box pays for it.
--
--  Charging only one of the two is the classic way to blow a margin quietly:
--  weights alone and the budget overspends, EV alone and the odds stop matching
--  the published table. The floor anchor's stock-weighted mean includes bonuses
--  for the same reason.
--
--  est_value stays the OBJECT's value, so nothing that reads it (scrap, the
--  catalogue page, the reveal's "worth") starts quoting an inflated number. The
--  bundle rides alongside as `bonus_value` / `bonus_tier` / `bonus_pct`.
--
--  Vouchers are issued at BOTH physical award sites -- the main prize and the
--  consolation object -- because in a catalogue like this most of the junk is
--  reached through the floor anchor, and bundling only the main pool would have
--  missed the items the idea exists for.
-- ---------------------------------------------------------------------------

ALTER TABLE public.items
  ADD COLUMN IF NOT EXISTS bonus_voucher_tier TEXT
    CHECK (bonus_voucher_tier IN ('tier_0','tier_1','tier_2','tier_3')),
  ADD COLUMN IF NOT EXISTS bonus_voucher_pct NUMERIC
    CHECK (bonus_voucher_pct > 0 AND bonus_voucher_pct <= 1);

-- Half a bundle is not a bundle: a tier without a percentage (or the reverse)
-- would price at 0 in box_odds and then issue nothing, or issue a NULL-pct
-- voucher that violates the vouchers CHECK at award time -- i.e. it would throw
-- on a winning spin rather than at configuration time.
ALTER TABLE public.items DROP CONSTRAINT IF EXISTS items_bonus_pair_ck;
ALTER TABLE public.items ADD CONSTRAINT items_bonus_pair_ck CHECK (
  (bonus_voucher_tier IS NULL AND bonus_voucher_pct IS NULL) OR
  (bonus_voucher_tier IS NOT NULL AND bonus_voucher_pct IS NOT NULL)
);
