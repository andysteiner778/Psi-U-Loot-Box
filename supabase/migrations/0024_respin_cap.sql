-- ============================================================================
--  HOUSE LOOT — 0024  A BOX IS NOT A RE-ROLL MACHINE
-- ============================================================================
--  The EV solve values the free-respin anchor at the full box price C. So when
--  a tier's items average LESS than C, the arithmetic concludes that paying in
--  re-rolls beats paying in prizes: it drives the item scale factor to zero and
--  produces a box that is 0% items and 65% "roll again". Exactly on budget, and
--  completely broken as a game.
--
--  This is live, not theoretical. Tier 3's average item currently sits a few
--  dollars ABOVE its $50 price. Win the MCAT books and Monitor 1 and the
--  average drops below it, and the top box silently stops handing out prizes
--  for the rest of the night -- with no error, no warning, and every gate
--  still green, because the books balance perfectly.
--
--  Found while modelling a re-pricing the owner asked about: dropping ordinary
--  items to 85% of value took Tier 3 from 62% prizes to 0%.
--
--  The real flaw is that a free re-roll is not worth C -- it is worth `target`,
--  the EV of a roll -- and pricing it at the sticker price is what lets it beat
--  real prizes. Re-deriving the anchor system days before the party is the
--  riskier fix, so this caps the share of a box that may be re-rolls (default
--  25%) and accepts the underspend, which surfaces honestly as a higher
--  realized margin in `npm run audit` rather than as a dead tier.
-- ============================================================================

UPDATE public.config
   SET value = value || jsonb_build_object('max_respin_share', 0.25)
 WHERE key = 'settings' AND NOT (value ? 'max_respin_share');
