-- ============================================================================
--  HOUSE LOOT — 0017  PROGRESSIVE SHARD DIFFICULTY
-- ============================================================================
--  A flat shard rate makes a four-piece set feel uniformly far away. Weighting
--  it so the first two come easily and the last one barely at all puts players
--  at 2-of-4 early, able to see the machine -- which is the state that keeps a
--  room spending, and the whole reason the shard track exists.
--
--  Curve [1, 1, 0.5, 0.3] against a 35% base on tier 3:
--
--      shard 1   35%   ~$143     1 in 3 rolls
--      shard 2   35%   ~$143
--      shard 3   17.5% ~$286
--      shard 4   10.5% ~$476     the wall
--                      -------
--                       ~$1,048  the owner's agreed target
--
--  box_odds now takes the player, because the shard EV charged to a roll must
--  be the one THAT player actually faces. Charging a flat rate would overcharge
--  someone near the end and undercharge someone starting out -- the anchor
--  would be priced against odds nobody has.
--
--  The parameter is OPTIONAL and defaults to NULL, so the admin odds preview
--  and any player-agnostic caller still get the base rates.
-- ============================================================================

UPDATE public.config SET value = value || jsonb_build_object(
  'shard_probs', jsonb_build_object('tier_1', 0.035, 'tier_2', 0.14, 'tier_3', 0.35),
  'shard_progress_curve', jsonb_build_array(1.0, 1.0, 0.5, 0.3)
) WHERE key = 'settings';

-- The single-argument version must GO, not merely be replaced. Adding an
-- overload with a defaulted second parameter makes box_odds('tier_1')
-- ambiguous, and Postgres refuses to choose -- every caller breaks at once.
