-- ============================================================================
--  HOUSE LOOT — 0028  REWARD ITEMS: FREE SPINS, DISCOUNTS, HOUSE CREDIT
-- ============================================================================
--  Three things to keep somebody rolling after a bad run: a free spin on a
--  dearer box, half off a dearer box, and plain house credit.
--
--  They are rows in `items`, which is the whole trick. The alternative was a
--  vouchers table plus a redemption step inside the price derivation in
--  open_box -- and "client-supplied box price" was the original security hole
--  in the spec, so that is the last code path to be improvising in days before
--  the party. As catalogue rows they instead get, for free:
--
--    * correct pricing -- the solvency engine already charges est_value to the
--      payout budget, so a $15 free spin costs the tier $15 of budget and its
--      drop rate falls out of the same arithmetic as everything else;
--    * honest odds -- they appear in the published loot table like any prize,
--      rather than being an invisible bonus nobody can verify;
--    * a hard cap -- `stock_qty` limits how many the house can ever hand out;
--    * an admin form that already edits them.
--
--  `reward_credit` is the marker. When a won item has one, the balance is
--  credited and nothing goes on a shelf.
--
--  Reported to the client as `respin`, which already means "credit came back,
--  roll again" and which the reveal, inventory and ticker all render today. A
--  fourth payload type would have meant touching all three for no gain.
--
--  BOTH win paths handle it: the ordinary draw AND the admin drop-override,
--  which builds its own payload. Missing the second one meant that forcing a
--  reward in order to test it exercised the wrong branch entirely and filed
--  the credit on the shelf as if it were an object.
-- ============================================================================

ALTER TABLE public.items
  ADD COLUMN IF NOT EXISTS reward_credit NUMERIC;

COMMENT ON COLUMN public.items.reward_credit IS
  'If set, winning this row credits the balance by this amount instead of '
  'granting a physical object. Used for free-spin, discount and credit rewards.';
