-- ---------------------------------------------------------------------------
--  0036 — the consolation prize must pay what it says
--
--  TWO BUGS IN THE FLOOR ANCHOR, both of the same family: the odds table
--  promised one thing and open_box did another.
--
--  1. REWARD ITEMS WERE HANDED OVER AS OBJECTS. The filler pool is drawn from
--     cheaper tiers, and those tiers contain house credit, free spins and
--     discount vouchers -- rows in `items` that are promises, not things. The
--     floor branch inserted whatever it drew as kind='physical', so a player
--     who lost a $10 roll got an inventory row literally named "$1 House
--     Credit" and never received the dollar. Measured against the live database
--     before this fix: 43 of 400 tier_2 rolls -- about 11% -- ate a reward that
--     way ($1 and $3 House Credit, FREE $3 SPIN, 50% OFF a $3 box, 50% OFF a
--     $10 box). Nothing threw; the player was simply short-changed.
--
--     They stay in the pool. A losing roll that pays credit or a free spin is
--     the retention the cheap tiers are built on, and box_odds prices them into
--     the floor anchor either way. They are just PAID correctly now.
--
--  2. TIER 1 PUBLISHED FILLER IT COULD NEVER AWARD. The branch was gated on
--     `p_box_tier IN ('tier_2','tier_3')`, but box_odds publishes a filler list
--     and floor_kind='item' for tier_1 too -- four items worth 4.4% of that
--     box. Those rolls silently paid credit instead, against a budget that had
--     already been priced as if an object were handed over. Now gated on
--     `p_box_tier <> 'tier_0'`; tier_0 has no cheaper tier to borrow from, so
--     its floor_kind is 'coins' and it never enters this branch anyway.
--
--  Same family as the shard odds being published without p_user_id and as
--  box_odds/open_box using different filler pools: the number shown was not the
--  number played.
-- ---------------------------------------------------------------------------
