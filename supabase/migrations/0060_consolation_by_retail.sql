-- ============================================================================
--  0060  THE CONSOLATION PRIZE IS CHOSEN BY RETAIL, AND MUST BE CHEAP
-- ============================================================================
--
--  When a roll misses the box's own prizes it pays a consolation: an object
--  borrowed from a cheaper tier. Because it fires on most misses, box_odds
--  charges it to the budget FIRST and spends only what is left on real prizes.
--
--  It had to be worth at least filler_min_frac (10%) of the box, measured in
--  est_value. Most of this catalogue is deliberately priced at $0.01, so the
--  only items that cleared the bar were the handful the owner had actually
--  valued -- the Desk, the Weed hat, the Seat Massager. The consolation was the
--  most expensive thing a cheaper tier had:
--
--      Good Stuff   budget $0.54, consolation cost $0.83 -> its own 12 items
--                   published at exactly 0%, and every roll lost money
--      High Roller  budget $5.40, consolation cost $4.38 -> its own prizes
--                   were 16% of rolls, a Chair or a Desk the other 84%
--
--  Now: RETAIL >= filler_min_retail_frac (50%) of the box, so the consolation
--  looks worth having, and est_value <= filler_max_cost_frac (10%) of it, so it
--  costs the house little. Those two numbers are decoupled in this catalogue on
--  purpose -- it is the whole reason retail is recorded.
--
--  Modelled against the live catalogue, 40 simulated parties of 15 players:
--  High Roller pays its own prizes on ~50% of rolls instead of ~16-24%, and the
--  house stayed profitable in every party.
--
--  Both functions change together. box_odds decides what the consolation is
--  WORTH; open_box decides what is HANDED OVER. If their predicates disagree,
--  the server awards something the published odds never priced.
--
--  Bodies below are the 0058/0054 definitions verbatim, with only the filler
--  predicate replaced. filler_min_frac stays in config, unread, so rolling back
--  is a matter of restoring the old predicate.
-- ============================================================================

UPDATE public.config
   SET value = value || jsonb_build_object('filler_min_retail_frac', 0.5, 'filler_max_cost_frac', 0.10)
 WHERE key = 'settings';
...
BEGIN
  ...
  -- A STANDING DISCOUNT, applied before any flash sale.
  --
  -- Separate from flash_sale on purpose: the sale is a timed event the player
  -- is told about, this is the owner quietly deciding the whole shop is priced
  -- too high and turning everything down until the room bites. They STACK --
  -- 40% off standing plus 20% off in a sale is 0.6 x 0.8 = 52% off, not 60% --
  -- because compounding two discounts can never drive a price below zero,
  -- whereas adding them can.
  v_c := ROUND(v_c * (1 - LEAST(0.95, GREATEST(0,
                COALESCE((cfg->>'extra_discount_pct')::NUMERIC, 0)))), 2);
  ...
  -- A box must still cost SOMETHING. At 95% off a $0.50 box rounds to $0.03;
  -- without a floor a deep enough discount rounds to $0.00 and the box becomes
  -- an infinite free spin that still pays out prizes.
  v_c := GREATEST(0.01, v_c);
  ...
  SELECT COALESCE(SUM(t.stock_qty), 0),
   ...
   WHERE p_box_tier <> 'tier_0'
     ...
     -- 0060: the consolation must LOOK worth having (retail) and COST the house
     -- little (est_value). See the header of 0060_consolation_by_retail.sql.
     AND COALESCE(NULLIF(t.msrp, 0), t.est_value)
         >= COALESCE((cfg->>'filler_min_retail_frac')::NUMERIC, 0) * v_c
     AND t.est_value <= COALESCE((cfg->>'filler_max_cost_frac')::NUMERIC, 1) * v_c
     AND t.is_active AND t.stock_qty > 0 AND t.est_value > 0
     -- A bundle-only row is a rider, not a prize: it reaches a player attached
     -- to something else (bonus_item_id) and must never be drawn by itself.
     AND NOT COALESCE(t.bundle_only, FALSE)
  ...
  IF (odds->>'floor_kind') = 'item' AND p_box_tier <> 'tier_0' THEN
    ...
    SELECT id, name, image_url, rarity, est_value, scrap_value, msrp
     ...
     WHERE array_position(ARRAY['tier_0','tier_1','tier_2','tier_3'], box_tier)
       ...
       -- 0060: must match the filler pool in box_odds exactly.
       AND COALESCE(NULLIF(msrp, 0), est_value)
           >= COALESCE((cfg->>'filler_min_retail_frac')::NUMERIC, 0) * (odds->>'box_price')::NUMERIC
       AND est_value <= COALESCE((cfg->>'filler_max_cost_frac')::NUMERIC, 1) * (odds->>'box_price')::NUMERIC
       ...
       -- ...and the consolation must be an object, matching the filler pool in
       -- box_odds. If these two predicates disagree, open_box awards something
       -- the published odds never priced -- the failure this codebase has hit
