-- ---------------------------------------------------------------------------
--  0040 — open_box refuses a locked tier
--
--  0039 added tier_lock_state(). This is the half that matters: without it the
--  lock is only advisory, and a client that ignores the flag (or a stale page
--  loaded before the last unit went) still gets charged for a box that can only
--  refund. The check sits after the profile row lock and before the balance is
--  touched, so a refused roll costs nothing and burns no voucher.
-- ---------------------------------------------------------------------------
