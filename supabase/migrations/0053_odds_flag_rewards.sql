-- ---------------------------------------------------------------------------
--  0053 — the odds screen must say which rows are things and which are promises
--
--  House credit, FREE SPIN and % OFF rows live in `items` on purpose, so the
--  engine prices them and `stock_qty` caps how many can ever be given away. But
--  they are not objects, and the odds screen listed them under "Physical Loot
--  Pool", counted them in the prize count, and folded them into the rarity
--  bands. That is what made the junk/floor odds look far higher than they are:
--  a player reading the screen could not tell a Ti-83 from a $3 credit.
--
--  `box_odds` now publishes `is_reward` on every item and filler entry, so the
--  UI can split them without re-deriving the predicate client-side and drifting
--  from the server's own definition — the exact failure mode that has bitten
