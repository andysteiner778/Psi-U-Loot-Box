-- ============================================================================
--  HOUSE LOOT — 0019  A FOURTH, CHEAPER TIER
-- ============================================================================
--  A $1 box, below the existing three, so someone who does not want to spend
--  can still play. Its payout budget is only ~88c, so it holds the genuinely
--  small stuff -- and, through the cross-tier rule, a very long shot at
--  something real.
--
--  Tier bands re-cut to this catalog's actual distribution:
--
--      tier_0  $1 box    under $3     the $1 and $2 oddments
--      tier_1  $5 box    $3 - $14
--      tier_2  $20 box   $15 - $49
--      tier_3  $50 box   $50+
--
--  Shard rates stay proportional to box price, so no tier is a cheaper route to
--  a shard than any other -- otherwise everyone farms whichever one is
--  mispriced and the PC's cost collapses.
-- ============================================================================

ALTER TABLE public.items DROP CONSTRAINT IF EXISTS items_box_tier_check;
ALTER TABLE public.items ADD CONSTRAINT items_box_tier_check
  CHECK (box_tier IN ('tier_0','tier_1','tier_2','tier_3'));

ALTER TABLE public.rolls DROP CONSTRAINT IF EXISTS rolls_box_tier_check;
ALTER TABLE public.rolls ADD CONSTRAINT rolls_box_tier_check
  CHECK (box_tier IN ('tier_0','tier_1','tier_2','tier_3'));

UPDATE public.config SET value = value || jsonb_build_object(
  'box_prices',  jsonb_build_object('tier_0', 1, 'tier_1', 5, 'tier_2', 20, 'tier_3', 50),
  'shard_probs', jsonb_build_object('tier_0', 0.007, 'tier_1', 0.035, 'tier_2', 0.14, 'tier_3', 0.35)
) WHERE key = 'settings';

-- Re-cut every item into the new bands.
UPDATE public.items SET box_tier =
  CASE WHEN est_value >= 50 THEN 'tier_3'
       WHEN est_value >= 15 THEN 'tier_2'
       WHEN est_value >= 3  THEN 'tier_1'
       ELSE 'tier_0' END
WHERE TRUE;
...
-- open_box must pass the caller through, or players would be charged the base
-- shard EV while facing their own curved odds.
CREATE OR REPLACE FUNCTION public.open_box(
  p_user_id        UUID,
  p_box_tier       TEXT,
  p_client_roll_id UUID DEFAULT NULL
...
BEGIN
  ...
  -- Pass the caller: shard odds now depend on how many shards they hold, and
  -- the EV charged to this roll must be the one THIS player actually faces.
