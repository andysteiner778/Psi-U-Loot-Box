-- ---------------------------------------------------------------------------
--  0058 — a standing discount the owner can dial, on top of the flash sale
--
--  "Add an option so I can further discount the boxes, one that stacks with the
--  fire sale button... want a slider so I can set boxes to be 80% off if no one
--  seems to be liking these prices."
--
--  `extra_discount_pct` is a standing markdown on every box, applied before the
--  timed flash sale so the two STACK. They compound rather than add: 40% off
--  standing plus 20% off in a sale is 0.6 x 0.8 = 52% off. Adding them could
--  drive a price to zero or below; compounding cannot.
--
--  Capped at 95%, and the final price is floored at $0.01. Without the floor a
--  deep discount rounds a $0.50 box to $0.00 and it becomes a free spin that
--  still hands out prizes and still charges the payout budget.
--
--  Everything downstream already reads the discounted price: `v_c` is what the
--  EV target, the respin refund, the scrap coin count and the published
--  `box_price` are all derived from, and open_box reads `box_price` back out of
--  box_odds rather than recomputing it.
-- ---------------------------------------------------------------------------
