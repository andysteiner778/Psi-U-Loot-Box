-- ============================================================================
--  HOUSE LOOT — 0013  CROSS-TIER DROPS
-- ============================================================================
--  Boxes were strictly partitioned: a $5 crate could only ever produce $5
--  items. That is the opposite of why people open cases -- the whole appeal is
--  that the cheap box MIGHT contain something absurd.
--
--  Every box now draws from the whole catalog, with off-tier prizes multiplied
--  by `cross_tier_factor` (0.15). The weight formula already makes an expensive
--  item rare in a cheap box -- min(cap, C*f/V) is 0.7% for a $150 item in a $5
--  box -- and the factor pushes it to roughly 0.1%. Rare, not impossible.
--
--  Cheap off-tier items stay excluded from the main pool because they are the
--  floor anchor; counting them in both places would double their frequency.
--
--  Solvency is unaffected: every item's value is still charged to the same EV
--  budget, so a lucky $5 box paying out $150 is funded by the ones that did not.
-- ============================================================================
