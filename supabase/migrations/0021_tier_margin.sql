-- ============================================================================
--  HOUSE LOOT — 0021  PER-TIER MARGIN (THE $1 BOX RUNS AT 1:1)
-- ============================================================================
--  The $1 box exists to clear junk and to get people rolling, not to earn. At
--  the house margin of 12.5% a dollar box returns 87.5c of mostly sub-$5
--  oddments, which is both a bad deal and a bad time. It now runs at 1:1: every
--  dollar in comes back out as goods.
--
--  Implemented as a PARTIAL `tier_margins` map rather than a full one, so that
--  changing `house_margin` still moves every tier that has no explicit override.
--
--  This is a two-engine change. `lib/economy.ts` is where solvency is PROVEN;
--  `box_odds` is where it is ENFORCED. A change to one and not the other is
--  invisible to every gate except the drift check in scripts/verify-sql.ts,
--  which is why that check exists.
-- ============================================================================

-- Write the override into the live settings row without disturbing any other
-- key. jsonb_set with create_if_missing, not a wholesale replace: this row
-- holds pc_shards_minted, which open_box updates concurrently.
UPDATE public.config
   SET value = jsonb_set(value, '{tier_margins}', '{"tier_0": 0}'::JSONB, TRUE)
 WHERE key = 'settings'
   AND NOT (value ? 'tier_margins');
