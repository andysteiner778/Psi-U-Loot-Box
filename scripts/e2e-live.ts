...
async function main() {
  ...
  section('discount vouchers discount the right box, once');
  {
    ...
    try {
      ...
      // Defensive: a roll that errors returns no roll_id, and dereferencing null
      // here threw out of the section entirely -- skipping its cleanup and
      // leaving probe accounts behind, which then failed a LATER assertion with
      // no hint as to why.
      const chargeOf = async (rollId: string | undefined) => {
        if (!rollId) return NaN;
        const { data } = await db.from('rolls').select('box_price').eq('id', rollId).maybeSingle();
        return data ? Number((data as { box_price: number }).box_price) : NaN;
      };
      ...
      // Any ordinary in-stock row will do. This used to demand stock_qty > 2,
      // which stopped existing the moment the catalogue was re-priced -- and the
      // whole section then died on a null rather than failing an assertion.
      const { data: plain } = await db
        ...
        .limit(1)
        .maybeSingle();
      ok(!!plain, 'found an ordinary item to force for the voucher tests');
      ...
      /* best effort; the cleanup section below re-checks */
      }
    }
